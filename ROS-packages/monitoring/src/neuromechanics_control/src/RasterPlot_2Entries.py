...
from neuromechanics_control.cfg import DynParameters_2EntriesConfig
...
class RasterplotAnimation(matplotlib.animation.TimedAnimation):
	...
	def new_figure(self):
		...
		self.first_spike_subscriber = rospy.Subscriber(self.first_inputTopic, neuromechanics_control.msg.Spike_group, self.FirstSpikeCallback, queue_size=None)
		self.second_spike_subscriber = rospy.Subscriber(self.second_inputTopic, neuromechanics_control.msg.Spike_group, self.SecondSpikeCallback, queue_size=None)
	...
	def __init__(self):
		...
		if self.use_sim_time:
 			self.clock_sub = rospy.Subscriber("/clock", Clock, self.callback, queue_size=1000)
		...
		# being showed.
		self.first_spike_subscriber = rospy.Subscriber(self.first_inputTopic, neuromechanics_control.msg.Spike, self.FirstSpikeCallback, queue_size=None)
		self.first_spike_subscriber.unregister()

		self.second_spike_subscriber = rospy.Subscriber(self.second_inputTopic, neuromechanics_control.msg.Spike, self.SecondSpikeCallback, queue_size=None)
		self.second_spike_subscriber.unregister()
