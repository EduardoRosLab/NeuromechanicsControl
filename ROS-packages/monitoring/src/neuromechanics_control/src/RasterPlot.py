...
class RasterplotAnimation(matplotlib.animation.TimedAnimation):
	...
	def new_figure(self):
		...
		self.spike_subscriber = rospy.Subscriber(self.inputTopic, neuromechanics_control.msg.Spike_group, self.SpikeCallback, queue_size=None)
	...
	def __init__(self):
		...
		self.master_clock_topic = rospy.get_param("master_clock_topic")
		...
		if self.use_sim_time:
 			self.clock_sub = rospy.Subscriber(self.master_clock_topic, Clock, self.callback, queue_size=1000)
		...
		# being showed.
		self.spike_subscriber = rospy.Subscriber(self.inputTopic, neuromechanics_control.msg.Spike, self.SpikeCallback, queue_size=None)
		self.spike_subscriber.unregister()
