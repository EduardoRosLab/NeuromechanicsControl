...
private:
	...
	neuromechanics_control::AnalogCompact msgPosition, msgVelocity;
...
public:
	Splitter (std::vector<std::string> & joint_list,
		...
		this->position_joint_publisher = nh.advertise<neuromechanics_control::AnalogCompact>(position_joint_topic, 1);
		ROS_DEBUG("Joint State Splitter: Writing position to topic %s", position_joint_topic.c_str());

		this->velocity_joint_publisher = nh.advertise<neuromechanics_control::AnalogCompact>(velocity_joint_topic, 1);
...
int main(int argc, char **argv)
{
	...
	std::string input_topic, position_joint_topic, velocity_joint_topic, clock_topic, master_clock_topic;
	std::vector<std::string> joint_list;
	bool sim_time;
 ...
 ros::WallRate init_rate(1.0);
	...
	if (sim_time){
		...
		objSplitter.clock_subscriber = nh.subscribe(master_clock_topic, 1000, &ExternalClock::ClockCallback, &(objSplitter.ext_clock));
