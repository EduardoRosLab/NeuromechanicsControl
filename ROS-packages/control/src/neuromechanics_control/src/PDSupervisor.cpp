...
//
//
// #include "neuromechanics_control/PDSupervisor.h"
// #include "neuromechanics_control/AnalogCompact.h"
// #include "neuromechanics_control/AnalogCompactDelay.h"
...
//
//
// void PDSupervisor::CurrentPositionCallback(const neuromechanics_control::AnalogCompactDelay::ConstPtr& msg){
...
// }
//
// void PDSupervisor::CurrentVelocityCallback(const neuromechanics_control::AnalogCompactDelay::ConstPtr& msg){
// 	// Check if the message is newer than previous ones.
...
//
//
// 	this->torque_command_pub = this->NodeHandler.advertise<neuromechanics_control::AnalogCompact>(torque_topic, 1e9);
...
//
// 	neuromechanics_control::AnalogCompact analog;
//
...
void PDSupervisor::CurrentPositionCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	// Check if the message is newer than previous ones.
	if (msg->header.stamp>this->current_position_time){
		this->current_position_time = msg->header.stamp;
		for (unsigned int i=0; i<this->joint_list.size(); ++i){
...
void PDSupervisor::CurrentVelocityCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	// Check if the message is newer than previous ones.
	if (msg->header.stamp>this->current_velocity_time){
		this->current_velocity_time = msg->header.stamp;
		for (unsigned int i=0; i<this->joint_list.size(); ++i){
