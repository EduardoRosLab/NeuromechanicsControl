...
private:
	...
	neuromechanics_control::AnalogCompact torqueMsg;
...
public:
	torqueRemaping (bool sim_time, std::string output_topic):
		...
		this->torqueRemaping_publisher = nh.advertise<neuromechanics_control::AnalogCompact>(output_topic, 10);
...
int main(int argc, char **argv)
{
	...
	std::string input_topic, output_topic, master_clock_topic;
	bool sim_time;
	ros::Publisher time_publisher;
 ...
 ros::Subscriber input_subscriber= nh.subscribe(baxterStateOptions);
	...
	if (sim_time){
		//ROS_DEBUG("Wireless Robot State: Subscribing to topic /clock");
		objtorqueRemaping.clock_subscriber = nh.subscribe(master_clock_topic, 1000, &ExternalClock::ClockCallback, &(objtorqueRemaping.ext_clock));
	}
