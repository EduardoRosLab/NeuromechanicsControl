...
#include "neuromechanics_control/ROSTopicInputDriver.h"
...
void ROSTopicInputDriver::SpikeCallback(const neuromechanics_control::Spike_group::ConstPtr& msg){
...
ROSTopicInputDriver::ROSTopicInputDriver(string TopicName, unsigned int MaxSpikeBuffered, double InitSimulationRosTime): NodeHandler(), CallbackQueue(), InitSimulationRosTime(InitSimulationRosTime), Net(0), Queue(0) {
	...
	ros::SubscribeOptions subscriberOptions = ros::SubscribeOptions::create<neuromechanics_control::Spike_group>(
	 		TopicName, MaxSpikeBuffered, boost::bind(&ROSTopicInputDriver::SpikeCallback, this, _1), ros::VoidPtr(), &this->CallbackQueue);
