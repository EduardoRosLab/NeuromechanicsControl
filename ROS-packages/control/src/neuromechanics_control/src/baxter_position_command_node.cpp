...
public:
	...
	void EventCallback(const neuromechanics_control::AnalogCompact::ConstPtr& trq){
	...
	void ControlEventCallback(const neuromechanics_control::LearningState::ConstPtr& lear){
...
// Subscriber with transport Options
ros::SubscribeOptions baxterStateOptions =
ros::SubscribeOptions::create<neuromechanics_control::AnalogCompact>(
	input_topic, 1000, boost::bind(&Synchronizer::EventCallback, &objSynchronizer, _1), ros::VoidPtr(), &CallbackQueue);

// baxterStateOptions.transport_hints = ros::TransportHints().unreliable().reliable().tcpNoDelay(true);
baxterStateOptions.transport_hints = ros::TransportHints().udp();
