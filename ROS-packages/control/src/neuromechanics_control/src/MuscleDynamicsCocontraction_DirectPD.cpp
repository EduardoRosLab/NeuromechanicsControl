/***************************************************************************
 *                  MuscleDynamicsCocontraction_DirectPD.cpp        *
 *                           -------------------                           *
...
void MuscleDynamicsCocontraction_DirectPD::CurrentPositionCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
...
void MuscleDynamicsCocontraction_DirectPD::CurrentVelocityCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	// Check if the signal should have been processed previously
...
void MuscleDynamicsCocontraction_DirectPD::DesiredPositionCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	// Check if the signal should have been processed previously
...
void MuscleDynamicsCocontraction_DirectPD::DesiredVelocityCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	// Check if the signal should have been processed previously
...
void MuscleDynamicsCocontraction_DirectPD::CocontractionCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
...
int MuscleDynamicsCocontraction_DirectPD::FindJointIndex(std::vector<std::string> strvector, std::string name){
...
MuscleDynamicsCocontraction_DirectPD::MuscleDynamicsCocontraction_DirectPD(
	std::vector<double> kp_gain,
	std::vector<double> kd_gain,
	std::vector<double> PD_min_output,
	std::vector<double> PD_max_output,
				...
				PD_max_output(PD_max_output)
{
	...
	this->subscriber_current_position = this->NodeHandler.subscribe(current_position_topic, 10.0, &MuscleDynamicsCocontraction_DirectPD::CurrentPositionCallback, this);
	this->subscriber_current_velocity = this->NodeHandler.subscribe(current_velocity_topic, 10.0, &MuscleDynamicsCocontraction_DirectPD::CurrentVelocityCallback, this);
	this->subscriber_desired_position = this->NodeHandler.subscribe(desired_position_topic, 10.0, &MuscleDynamicsCocontraction_DirectPD::DesiredPositionCallback, this);
	this->subscriber_desired_velocity = this->NodeHandler.subscribe(desired_velocity_topic, 10.0, &MuscleDynamicsCocontraction_DirectPD::DesiredVelocityCallback, this);

	this->subscriber_cocontraction = this->NodeHandler.subscribe(cocontraction_input_topic, 10.0, &MuscleDynamicsCocontraction_DirectPD::CocontractionCallback, this);
...
void MuscleDynamicsCocontraction_DirectPD::UpdateTorque(ros::Time current_time){
	...
	for (unsigned int i=0; i<this->joint_list.size(); ++i){
		...
		if (this->pd_output[i] > this->PD_max_output[i]){
				this->pd_output[i] = this->PD_max_output[i];
		}
		else if (this->pd_output[i] < this->PD_min_output[i]){
				this->pd_output[i] = this->PD_min_output[i];
		}

		this->output_torque[i] = this->pd_output[i];
...
void MuscleDynamicsCocontraction_DirectPD::CleanQueue(std::priority_queue<neuromechanics_control::AnalogCompact, std::vector<neuromechanics_control::AnalogCompact>, analog_comparison > & queue,
