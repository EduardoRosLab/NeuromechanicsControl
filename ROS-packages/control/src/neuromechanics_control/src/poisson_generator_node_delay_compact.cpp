...
#include <neuromechanics_control/ROSPoissonGenerator.h>
...
public:
	...
	void AnalogCallback(const neuromechanics_control::AnalogCompact::ConstPtr& msg){
	...
	void ControlCallback(const neuromechanics_control::LearningState::ConstPtr& lear){
...
int main(int argc, char **argv)
{
  ...
  nh.getParam("use_sim_time", use_sim_time);
	...
	if (use_sim_time){
		ROS_DEBUG("Poisson Generator: Subscribing to topic /clock");   //change from "clock" to "clock_ext"
		clock_subscriber = nh.subscribe("/clock", 1000, &ExternalClock::ClockCallback, &ext_clock);   //change from "clock" to "clock_ext"
	...
	ros::Publisher output_publisher_group = nh.advertise<neuromechanics_control::Spike_group>(output_topic, 1e9);
		...
		if (use_sim_time){
			...
			if (new_time>time){
				...
				if (objSensorState.IsInLearning()){
					neuromechanics_control::Spike_group spike_group;
